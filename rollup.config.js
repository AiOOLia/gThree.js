...
export function wgsl() {
    ...
    return {
        ...
        transform( code, id ) {
            ...
            if ( /\.wgsl.js$/.test( id ) === false ) return;

            code = code.replace( /\/\* wgsl \*\/\`(.*?)\`/sg, function ( match, p1 ) {
...
function checkThree() {
	return {
		...
		renderChunk( code ) {
			...
			return `window.three = THREE ? THREE : window.three;
${ code }`;
...
function header() {
	...
	return {
		...
		renderChunk( code ) {
			...
			return `/**
 * @license
 * Copyright 2022-2022 gThree.js Authors
		...
		output: [
			{
				format: 'umd',
				name: 'gTHREE',
				file: 'build/gThree.js',
				indent: '\t'
			}
		]
	}/*,
	{
		input: 'src/Core/Three/Three.js',
		plugins: [
			addons(),
			glconstants(),
			glsl(),
