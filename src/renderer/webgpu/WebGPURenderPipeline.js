...
import {FrontSide, BackSide, DoubleSide,
    ...
    ZeroFactor, OneFactor, SrcColorFactor, OneMinusSrcColorFactor, SrcAlphaFactor, OneMinusSrcAlphaFactor, DstAlphaFactor, OneMinusDstAlphaFactor, DstColorFactor, OneMinusDstColorFactor, SrcAlphaSaturateFactor} from "three";
import {MathUtils} from "three";

class WebGPURenderPipeline {

	constructor( device, shaderSource, renderer, sampleCount ) {
		this.uuid = MathUtils.generateUUID();
		this.shaderAttributes = null;
		this.stageVertex = null;
		this.stageFragment = null;
		this.shaderSource = shaderSource;
		this.shaderSource.usedTimes++;
		this.bindingMaterials = new Set();
	...
	init( pipelineLayout, stageVertex, stageFragment, object, material, shaderSource ) {

		const geometry = object.geometry;

		// determine shader attributes

		const shaderAttributes = this._getShaderAttributes( shaderSource, geometry );
	...
	_getArrayStride( type, bytesPerElement ) {
		...
		if ( type === 'f32' || type === 'i32' || type === 'u32' ) return bytesPerElement;
		if ( type.startsWith('vec2') )  return bytesPerElement * 2;
		if ( type.startsWith('vec3') )  return bytesPerElement * 3;
		if ( type.startsWith('vec4') )  return bytesPerElement * 4;
	...
	_getPrimitiveState( object, material ) {
		...
		descriptor.topology = this._getPrimitiveTopology( object, material );
	...
	_getPrimitiveTopology( object, material ) {
		if ( object.isMesh ) {
	...
	_getVertexFormat( type, bytesPerElement ) {
		...
		if ( type === 'f32' ) return GPUVertexFormat.Float32;
		...
		if ( type === 'vec3<f32>' ) return GPUVertexFormat.Float32x3;
		...
		if ( type === 'i32' ) return GPUVertexFormat.Sint32;
		...
		if ( type === 'vec3<i32>' ) return GPUVertexFormat.Sint32x3;
		...
		if ( type === 'u32' ) return GPUVertexFormat.Uint32;
		...
		if ( type === 'vec3<u32>' ) return GPUVertexFormat.Uint32x3;
	...
	_getShaderAttributes( shaderSource, geometry ) {

		const nodeAttributes = shaderSource.attributes;
		const attributes = [];

		for ( let i = 0, ln = nodeAttributes.length; i < ln; i ++ ) {

			const nodeAttribute = nodeAttributes[ i ];

			const name = nodeAttribute.name;
			const type = nodeAttribute.type;
			const slot = nodeAttribute.location;

			const geometryAttribute = geometry.getAttribute( name );
			const bytesPerElement = ( geometryAttribute !== undefined ) ? geometryAttribute.bytesPerElement : 4;
