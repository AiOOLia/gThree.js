import { GPUTextureFormat, GPUAddressMode, GPUFilterMode, GPUTextureDimension, GPUTextureViewDimension } from './constants.js';
import {CubeTexture} from "three";
import {Texture} from "three";
import {NearestFilter, NearestMipmapNearestFilter, NearestMipmapLinearFilter, LinearFilter, RepeatWrapping, MirroredRepeatWrapping,
    RGBFormat, RGBAFormat, RedFormat, RGFormat, RGBA_S3TC_DXT1_Format, RGBA_S3TC_DXT3_Format, RGBA_S3TC_DXT5_Format, UnsignedByteType, FloatType, HalfFloatType,
...
class WebGPUTextures {
	...
	constructor( device, properties, info ) {
		...
		this.defaultTexture.minFilter = NearestFilter;
		this.defaultTexture.magFilter = NearestFilter;

		this.defaultCubeTexture = new CubeTexture();
		this.defaultCubeTexture.minFilter = NearestFilter;
		this.defaultCubeTexture.magFilter = NearestFilter;

		this.defaultTextureGPU = null;
		this.defaultCubeTextureGPU = null;
	...
	getDefaultTexture() {
		...
		if ( this.defaultTextureGPU === null ) {

			this._uploadTexture( this.defaultTexture );

			this.defaultTextureGPU = this.getTextureGPU( this.defaultTexture );

		}

		return this.defaultTextureGPU;

	}

	getDefaultCubeTexture() {

		if ( this.defaultCubeTextureGPU === null ) {

			this._uploadTexture( this.defaultCubeTexture );

			this.defaultCubeTextureGPU = this.getTextureGPU( this.defaultCubeTexture );

		}

		return this.defaultCubeTextureGPU;
	...
	getDefaultTextureView( texture ) {
		const textureProperties = this.properties.get( texture );

		return textureProperties.defaultView;
	}
	...
	_uploadTexture( texture ) {
		...
		if ( texture.isDataTexture || texture.isDataArrayTexture || texture.isData3DTexture ) {
			...
			if (image.nodeType && image.nodeType === 1 && image.nodeName === 'IMG') {
				...
				// assume HTMLCanvasElement or ImageBitmap

				this._copyExternalImageToTexture( image, textureGPU );
		...
		//
		if(!textureProperties.defaultView) {
			textureProperties.defaultView = textureGPU.createView( { dimension: this._getViewDimension(texture)} );
		}
		//
	...
	_copyCubeMapToTexture( images, format, texture, textureGPU, textureGPUDescriptor, needsMipmaps ) {
		...
		for ( let i = 0; i < 6; i ++ ) {
			...
			if ( image.isDataTexture ) {
			...
			} else if(image.nodeType && image.nodeType === 1 && image.nodeName === 'IMG'){
				...
				this._copyExternalImageToTexture( image, textureGPU, { z : i } );
